[:section#intro
 ...
 equivalence, etc."]
   ...
   [:li [:strong "Optimization"] " We may precompute some static branch of a
 function, or alter it for more efficiency."]

   [:li [:strong "Decomposition"] " We may pull out different pieces of a
 ...
 [:p [:em "trlisp"] " is an implementation of tree calculus with the small,
 regular grammar of Lisp. The element at the head of a list, the function
 position, is invoked, with the elements of the tail of the list providing
 the arguments. Unlike traditional Lisps, " [:em "trlisp"] " functions may be
 transparently decomposed so that the function may be inspected, analyzed,
 compared, optimized, decomposed, and modified, without the need for quotation.
 That is to say, trlisp functions are reflective."]
